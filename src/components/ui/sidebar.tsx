...
export const DesktopSidebar = ({ className, children, ...props }: React.ComponentProps<typeof motion.div>) => {
  ...
  return (
    <>
      <motion.div
        className={cn(
          'h-full px-4 py-4 hidden  md:flex md:flex-col bg-white w-[300px] flex-shrink-0 border',
          className
        )}
        animate={{
...
export const MobileSidebar = ({ className, children, ...props }: React.ComponentProps<'div'>) => {
  ...
  return (
    <>
      <div
        className={cn(
          'h-[57px] absolute z-50 px-4 py-4 flex flex-row md:hidden items-center justify-between bg-white border w-fit'
        )}
        {...props}
      >
